...
class DecisionMaker(object):
    """ Scheduler of the entire framework
        Attr:
            ...
            posterior_commit_threshold(float): in [0,1] a threshold to make a
                decision. this will be changed with decision functionality in
                further releases.
            query_method(query_method): specified query method for the decision
                maker. for further specifications check, query methods.
            momentum_alpha(float): the alpha parameter used to compute posterior
    ...
    def __init__(self, state='', len_query=4,
                 query_method=MomentumQuerying(alp=alphabet()),
                 alp=alphabet(), is_txt_sti=True, momentum_alpha=0
                 , artificial_stop=False):
        self.state = state
        ...
        self.min_num_seq = 10
        self.max_num_seq = 20
        self.posterior_commit_threshold = .85
        self.momentum_alpha = momentum_alpha

        self.query_method = query_method

        self.artificial_stop = artificial_stop
    ...
    def decide(self, p):
        ...
        # Generate stopping criteria statistics
    ...
    def decide_state_update(self):
        ...
        idx = np.where(tmp == np.max(tmp))[0][0]
    ...
    def prepare_stimuli(self):
        ...
        stimuli = self.query_method.update_query(tmp[:])
        return stimuli
